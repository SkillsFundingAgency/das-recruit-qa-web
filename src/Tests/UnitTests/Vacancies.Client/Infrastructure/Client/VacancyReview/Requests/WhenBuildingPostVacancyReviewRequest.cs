...
public class WhenBuildingPostVacancyReviewRequest
{
    ...
    [Test, MoqAutoData]
    public void Then_The_Request_Is_Correctly_Built_And_Data_Populated_For_AcrossEngland(
        string fieldIdentifier,
        string fieldIdentifier2,
        [Frozen]Mock<IEncodingService> encodingService)
    {
        ...
        ((VacancyReviewDto)actual.Data).EmployerLocationOption.Should().Be(AvailableWhere.AcrossEngland);
        ((VacancyReviewDto)actual.Data).EmployerLocations.Should().BeEquivalentTo(new List<Address>());
        ((VacancyReviewDto)actual.Data).ManualQaFieldIndicators.Should().BeEquivalentTo([fieldIdentifier]);
